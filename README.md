...
however if you want to use external power => power via the micro c connector on esp32 with external 5v powerbank  and leave rockerswitch off <br />
# remarks
...
The issue is the power voltage ramps up too slow. The solution is using a ka75330zta powered by vin and the output to enable pin of the esp32. <br />
...
Solving the bad ramp up of the voltage (when power is supplied to the gripper when newly docked)to the esp which is supplied by the 24v from toolchanger with buck converter 5v to esp.<br />

Working already so long with esp controllers this issue of esp32 was new to me. Learning to use a 3pin voltage guard like the ka72330 to prevent freezing is an important feature to implement on your controller when making projects with the esp32.
